...
public class Player extends Entity implements Battleable {
    ...
    public boolean build(String entity, EntityFactory factory, GameMap map) {
        ...
        if (item instanceof MidnightArmour && hasZombiesInDungeon(map)) return false;
        return inventory.add(item);
    }

    public void useSceptre(GameMap map, Enemy enemy) {
        Sceptre sceptre = inventory.getFirst(Sceptre.class);
        if (sceptre != null) {
            sceptre.controlEnemy(map.getGame(), enemy);
        }
    }
    ...
    public void triggerNext(int currentTick) {
        ...
        state.transitionToNextState(inEffective);
    ...
    private boolean hasZombiesInDungeon(GameMap map) {
